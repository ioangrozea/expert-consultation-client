...
export class UsersEffects {
  ...
  loadUsers$ = this.actions$.pipe(
    ...
    switchMap(() => {
      return this.usersService.list().pipe(
        map(
          (usersPage: Page<User>) =>
            new usersActions.LoadUsersSuccess(usersPage)
        ),
  ...
  saveUser$ = this.actions$.pipe(
    ...
    switchMap((user: User) => {
      return this.usersService.save(user).pipe(
        ...
        catchError(error =>
          of(
            new usersActions.SaveUserFail(this.mapPayloadToUserSaveError(error))
          )
        )
      );
    })
  ...
  saveUsersExcel = this.actions$.pipe(
    ...
    switchMap((usersExcel: string) => {
      return this.usersService.saveUsersExcel(usersExcel).pipe(
        map(
          (savedUser: User[]) =>
            new usersActions.SaveUserExcelSuccess(savedUser)
        ),
